...
type ReviewDecision = "approved" | "rejected" | "request_reupload";

type AdminQueueItemResponse = {
  session: VerificationSessionResponse;
  user: AuthUser;
};
...
function WorkbenchPage({ onNavigate }: WorkbenchPageProps) {
  ...
  const [adminQueue, setAdminQueue] = useState<AdminQueueItemResponse[]>([]);
  const [isLoadingAdminQueue, setIsLoadingAdminQueue] = useState(false);
  const [isSubmittingDecisionFor, setIsSubmittingDecisionFor] = useState("");
  const [reviewReasonBySession, setReviewReasonBySession] = useState<Record<string, string>>({});
  ...
  const registerUser = async () => {
    ...
    if (authData.user.role === "admin" || authData.user.role === "reviewer") {
      await listFlaggedReviewQueue(authData.access_token);
    } else {
      setAdminQueue([]);
      setAdminMessage("");
    }
  ...
  const fetchMe = async () => {
    ...
    if (userData.role === "admin" || userData.role === "reviewer") {
      await listFlaggedReviewQueue(token);
    } else {
      setAdminQueue([]);
      setAdminMessage("");
    }
  ...
  const listFlaggedReviewQueue = async (tokenOverride?: string) => {
    const authToken = tokenOverride ?? token;
    if (!authToken) {
      setAdminMessage("No token available. Login first.");
      setAdminQueue([]);
      return;
    }
    ...
    setAdminMessage("");
    try {
      const response = await fetch(`${apiBase}/api/v1/admin/reviews/queue`, {
        headers: { Authorization: `Bearer ${authToken}` }
      });
      const data = (await response.json()) as AdminQueueItemResponse[] | { detail?: unknown };
      if (!response.ok) {
        throw new Error(getApiError(data, "Could not fetch admin review queue"));
      }
      setAdminQueue(data as AdminQueueItemResponse[]);
      setAdminMessage("Admin review queue loaded.");
    } catch (err) {
      setAdminMessage(err instanceof Error ? err.message : "Could not fetch admin review queue");
    } finally {
      setIsLoadingAdminQueue(false);
    }
  };

  const submitReviewDecision = async (sessionId: string, decision: ReviewDecision) => {
    if (!token) {
      setAdminMessage("No token available. Login first.");
      return;
    }

    const reason = (reviewReasonBySession[sessionId] ?? "").trim();
    if (decision !== "approved" && !reason) {
      setAdminMessage("Please provide a reason for reject/request re-upload.");
      return;
    }

    setIsSubmittingDecisionFor(sessionId);
    setAdminMessage("");
    try {
      const response = await fetch(`${apiBase}/api/v1/admin/reviews/${sessionId}/decision`, {
        method: "POST",
        headers: {
      ...
      const data = (await response.json()) as AdminDecisionResponse | { detail?: unknown };
      if (!response.ok) {
        throw new Error(getApiError(data, "Could not submit review decision"));
      }
      const resolved = data as AdminDecisionResponse;

      setAdminQueue((prev) =>
        prev.filter((item) => item.session.session_id !== resolved.session.session_id)
      );
      setReviewReasonBySession((prev) => {
        const next = { ...prev };
        delete next[resolved.session.session_id];
        return next;
      });
      setVerificationSessions((prev) =>
        prev.map((session) =>
          session.session_id === resolved.session.session_id ? resolved.session : session
        )
      );
      if (currentUser?.user_id === resolved.user.user_id) {
        setCurrentUser((prev) =>
          prev
            ? {
                ...prev,
                kyc_status: resolved.user.kyc_status,
                is_verified: resolved.user.is_verified
              }
            : prev
        );
      }
      setAdminMessage(resolved.message);
    } catch (err) {
      setAdminMessage(err instanceof Error ? err.message : "Could not submit review decision");
    } finally {
      setIsSubmittingDecisionFor("");
    }
  };
  ...
  const selectedVerificationSession =
    ...
    null;
  const canReviewSessions = currentUser?.role === "admin" || currentUser?.role === "reviewer";
  ...
  useEffect(() => {
    ...
    if (canReviewSessions) {
      void listFlaggedReviewQueue(token);
    }
  }, [token, canReviewSessions]);
  ...
  useEffect(() => {
    ...
    void fetchMe().catch((err: unknown) => {
      setAuthMessage(err instanceof Error ? err.message : "Could not load profile");
    });
  }, [token, currentUser]);
    ...
    <main className="page-shell">
      ...
      <section className="panel wide">
        <div className="panel-head">
          ...
          <span className="panel-tag">Step 7</span>
        </div>
        {!canReviewSessions ? (
          <p className="muted">
            Login with reviewer/admin role to access flagged-session decisions.
          </p>
        ) : (
          <>
            <p className="muted">
              Review flagged verification sessions and apply a final manual decision.
            </p>
            <div className="btn-row">
              <button
                className="btn secondary"
                onClick={() => void listFlaggedReviewQueue()}
                type="button"
                disabled={isLoadingAdminQueue}
              >
                {isLoadingAdminQueue ? "Loading Queue..." : "Load Review Queue"}
              </button>
            </div>
            ...
            <div className="quick-kpis">
              <div>
                ...
                <strong>{adminQueue.length}</strong>
              </div>
              <div>
                <small>Role</small>
            ...
            <div className="table-wrap">
              {adminQueue.length === 0 ? (
                <p className="muted">No flagged sessions pending manual review.</p>
              ) : (
                <table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {adminQueue.map((item) => (
                      <tr key={item.session.session_id}>
                        <td>
                          <strong>{item.user.full_name}</strong>
                          <br />
                          <span className="muted">{item.user.email}</span>
                        </td>
                        <td>{new Date(item.session.timestamp).toLocaleString()}</td>
                        <td>{item.session.match_score ?? "N/A"}</td>
                        <td>{item.session.liveness_score ?? "N/A"}</td>
                        <td>{item.session.deepfake_probability ?? "N/A"}</td>
                        <td>
                          <input
                            placeholder="Reason for reject/re-upload"
                            value={reviewReasonBySession[item.session.session_id] ?? ""}
                            onChange={(event) =>
                              setReviewReasonBySession((prev) => ({
                                ...prev,
                                [item.session.session_id]: event.target.value
                              }))
                            }
                          />
                        </td>
                        <td>
                          <div className="row-actions">
                            <button
                              className="btn small"
                              type="button"
                              disabled={isSubmittingDecisionFor === item.session.session_id}
                              onClick={() =>
                                void submitReviewDecision(item.session.session_id, "approved")
                              }
                            >
                              {isSubmittingDecisionFor === item.session.session_id
                                ? "Submitting..."
                                : "Approve"}
                            </button>
                            <button
                              ...
                              disabled={isSubmittingDecisionFor === item.session.session_id}
                              onClick={() =>
                                void submitReviewDecision(item.session.session_id, "rejected")
                              }
                            >
                              Reject
                            </button>
            ...
            <p className={adminMessage ? "muted" : ""}>
              {adminMessage || "No review actions yet."}
            </p>
          </>
        )}
      </section>
